...
function getTexasServiceLoader(slug) {
  if (slug.startsWith('dentist-')) {
    return () => import('../../data/services/states/texas/dentist.js');
  }

  if (slug.startsWith('healthcare-')) {
    return () => import('../../data/services/states/texas/healthcare.js');
  }

  return () => import('../../data/services/states/texas/general.js');
}
...
const TexasService = () => {
  ...
  useEffect(() => {
    ...
    getTexasServiceLoader(slug)()
      ...
      .catch((error) => {
        console.error(`Failed to load Texas service data for ${slug}:`, error);
        if (isMounted) {
          setService(null);
        }
      });
