// ✅ Single source of truth for all URLs — use www consistently everywhere
...
// ✅ Helper — always produces https://www.growlimo.com/path/
export const buildUrl = (path) => {
  if (path === '/') return `${WWW}/`;
  const clean = path.startsWith('/') ? path : `/${path}`;
  return clean.endsWith('/') ? `${WWW}${clean}` : `${WWW}${clean}/`;
};
