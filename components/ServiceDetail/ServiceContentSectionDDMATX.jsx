...
export default function ServiceContentSectionDDMATX({ service, slug, onSelectPlan }) {
  ...
  const heroImg = images[0] || '/images/services/hero-dentist-digital-marketing-agency-texas.webp';
  const strategyImg = images[1] || '/images/services/dentist-digital-marketing-agency-texas-strategy.webp';
  const keywordImg = images[2] || '/images/services/dentist-digital-marketing-agency-texas-results-1.webp';
  const linkImg = images[3] || '/images/services/dentist-digital-marketing-agency-texas-results-2.webp';
  const bannerImg = images[4] || '/images/services/dentist-digital-marketing-agency-texas-banner.webp';

  const areaServedName = service.areaServedName || service.areaServed?.name || 'Texas';
  const locationLabel = `${areaServedName.toUpperCase()} DENTAL MARKETING SPECIALISTS`;
  const servicesIntroHeading =
    service.servicesIntroHeading || 'Our Complete Dental Digital Marketing Services for Texas Practices';
  ...
  const parseTrustBarItem = (item, idx) => {
    ...
    if (cleaned.toLowerCase().includes('4.7x') || (cleaned.toLowerCase().includes('roi') && cleaned.toLowerCase().includes('marketing'))) {
      return { value: '4.7x', label: 'Marketing ROI' };
    }
    if (cleaned.toLowerCase().includes('296%')) {
      return { value: '296%', label: 'New Patient Inquiry Growth' };
    }
    if (cleaned.toLowerCase().includes('hipaa')) {
      return { value: 'HIPAA', label: 'Aware Configurations' };
    }
    if (cleaned.toLowerCase().includes('ymyl') || cleaned.toLowerCase().includes('dental')) {
      return { value: 'YMYL', label: 'Dental Content' };
    }
    if (cleaned.toLowerCase().includes('meta') && cleaned.toLowerCase().includes('partner')) {
      return { value: 'Meta', label: 'Business Partner' };
    }
  ...
  const heroTrustPills = trustItems.length >= 3
    ...
    : [
        { value: '4.7x', label: 'Marketing ROI' },
        { value: '296%', label: 'New Patient Inquiry Growth' },
  ...
  const overlayTrustCards = trustItems.length >= 3
    ...
    : [
        { value: '4.7x', text: 'Average Return on Digital Marketing Investment' },
        { value: '296%', text: 'Average New Patient Inquiry Growth' },
        { value: 'HIPAA-Aware', text: 'Analytics for Texas Dental Practices' },
      ];
  ...
  const problemSection = contentSections[0] || {};
  const specialistSection = contentSections[1] || {};
  const integratedSection = contentSections[2] || {};
    ...
    <div className="bg-[#080D18] font-sans selection:bg-[#00C68A]/30 selection:text-white overflow-x-hidden text-[#8FA8C8]">
            ...
            <div className="lg:col-span-6 w-full">
              <div className="bg-[#1A2438]/80 backdrop-blur-md border border-[rgba(255,255,255,0.08)] rounded-[20px] p-3 shadow-2xl relative">
                ...
                <h3 className="font-sora font-bold text-[14px] text-[#F0F4FF] mb-2 text-left border-b border-[rgba(255,255,255,0.06)] pb-1">
                  Free Texas Dental Marketing Audit
                </h3>
                <Form
                  slug={slug}
                  compact={true}
      ...
      <section className="relative w-full overflow-hidden">
        <div className="relative w-full h-[480px]">
          <img
            src={heroImg}
            alt="GrowLimo Dentist Digital Marketing Agency Texas"
            ...
            <div className="lg:col-span-5">
              <div className="bg-[#F8FAFC] border border-[#E3EEF7] rounded-[20px] p-8 shadow-sm">
                <h3 className="font-sora font-bold text-[18px] text-[#0B1829] mb-6 text-left border-b border-[#E3EEF7] pb-3">
                  Texas Dental Market Reality
                </h3>
                <div className="space-y-5">
                  {problemSection.bullets && problemSection.bullets.map((bullet, idx) => {
      ...
      <section className="bg-[#0C1220] py-[96px] relative z-10 border-t border-[rgba(255,255,255,0.04)]">
        <div className="container mx-auto px-4 md:px-10 max-w-[1100px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
            {specialistSection.bullets && specialistSection.bullets.map((bullet, idx) => {
              const parts = bullet.split('—');
              const title = parts[0] ? parts[0].trim() : '';
              const desc = parts[1] ? parts[1].trim() : '';
      ...
      {/* SECTION 4b: INTEGRATED MARKETING (white bg) */}
      <section className="bg-[#FFFFFF] py-[96px] relative z-10 border-t border-[#E3EEF7]">
        <div className="container mx-auto px-4 md:px-10 max-w-[1100px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
            {integratedSection.bullets && integratedSection.bullets.map((bullet, idx) => {
              const parts = bullet.split('—');
              const title = parts[0] ? parts[0].trim() : '';
              const desc = parts[1] ? parts[1].trim() : bullet;

              return (
                <div
                  key={idx}
                  className="bg-[#F8FAFC] border border-[#E3EEF7] rounded-[16px] p-6 shadow-sm transition-all duration-300 hover:border-[#00C68A]/30 flex gap-4 items-start text-left"
                >
                  <div className="w-[30px] h-[30px] rounded-full bg-[rgba(0,198,138,0.12)] border border-[rgba(0,198,138,0.20)] flex items-center justify-center shrink-0 mt-0.5">
                    <svg className="w-[14px] h-[14px] text-[#00C68A]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                  ...
                  <div className="flex-1">
                    <h3 className="font-sans text-[15px] font-bold text-[#0B1829] leading-tight mb-2">
                      {title}
                    </h3>
                    <p className="font-sans text-[14px] leading-relaxed text-[#3D5A73]">
                      {desc}
                    </p>
                  </div>
                </div>
      ...
      <section className="bg-[#F8FAFC] py-[96px] relative z-10 border-t border-[#E3EEF7]">
        <div className="container mx-auto px-4 md:px-10 max-w-[1100px]">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-5">
              <div className="rounded-[20px] overflow-hidden shadow-xl border-4 border-white relative group">
                <img
                  src={strategyImg}
                  alt="Texas Dental Digital Marketing Strategy"
            ...
            <section className={`${bgClass} py-[96px] relative z-10`}>
              <div className="container mx-auto px-4 md:px-10 max-w-[1100px]">
                <div className="max-w-[860px] mx-auto">
                  ...
                  {serviceSec.closingText && !(serviceSec.internalLinks && serviceSec.internalLinks.length > 0) && (
                    <div className={`mt-8 p-6 rounded-[16px] border-l-4 border-[#00C68A] text-left ${isWhiteBg ? 'bg-[#F8FAFC]' : 'bg-[#1A2438]/60'}`}>
                      {serviceSec.linkTo ? (
                        <Link
              ...
              <section className="relative w-full py-16 bg-[#080D18] flex justify-center">
                <div className="container max-w-[900px] px-4">
                  <div className="rounded-[20px] overflow-hidden shadow-2xl border border-[rgba(255,255,255,0.06)] relative group">
                    <img
                      src={linkImg}
                      alt="Texas Multi-Location Dental Marketing"
                      className="w-full h-auto max-h-[480px] object-cover transition-transform duration-500 group-hover:scale-[1.02]"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent pointer-events-none" />
                    <div className="absolute bottom-5 left-6 right-6 text-left">
      ...
      <section className="relative w-full overflow-hidden">
        <div className="relative w-full h-[320px]">
          <img
            src={bannerImg}
            alt="GrowLimo Texas Dentist Digital Marketing Results"
